...
Package manager — **pnpm** (workspace). Packages: `backend`, `frontend`.

Root script aliases (`pnpm b`, `pnpm f`) proxy commands to the corresponding package.
Shared scripts: `dev`, `build`, `preview`, `lint`, `format`, `typecheck`, `format:check`.
...
Use `pnpm dlx` for running CLI tools, if they are not available directly or not installed locally.
...
- Single quotes, semicolons, trailing commas, 2-space indentation, line length ≤ 100 characters.
- Added plugin trivago/prettier-plugin-sort-imports for consistent import sorting.
...
- Use the **Composition API** and **VueUse** for local state and logic in components.
...
- Use types **generated by Prisma** (from `backend/src/generated/prisma/`).
- Use pnpm dlx to run Prisma CLI commands (`pnpm dlx prisma migrate dev`).
- Command `db:deploy` runs migrations and generates Prisma client before starting the app.
